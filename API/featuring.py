...
def get_n_last_matches_against_each_other(matches, date, home_team, away_team,
                                          n=5):
    '''
    ...
    home_matches = matches[(matches['HomeTeam'] == home_team) & (
            matches['AwayTeam'] == away_team)]
    away_matches = matches[(matches['HomeTeam'] == away_team) & (
            matches['AwayTeam'] == home_team)]
...
def get_wins(matches, team):
    ...
    '''
    home_wins = matches[(matches.FTR == 1) & (matches.HomeTeam == team)].shape[
        0]
    away_wins = \
        matches[(matches.FTR == -1) & (matches.AwayTeam == team)].shape[0]
...
def coefficients_to_probability(matches):
    ...
    matches['profit_B365'] = sum(
        (1 / matches['B365H'], 1 / matches['B365D'], 1 / matches['B365A']))
    matches['profit_BbAv'] = sum(
        (1 / matches['BbAvA'], 1 / matches['BbAvD'], 1 / matches['BbAvH']))

    # Converting all betting coefficients into probabilities of homw/draw/away:
    for betting_column in ['B365H', 'B365D', 'B365A', 'BbAvH', 'BbAvD',
                           'BbAvA']:
        matches[betting_column] = 1 / (matches[betting_column] * matches[
            'profit_' + betting_column[:-1]])
...
def get_features_for_match(match, matches, n1=10, n2=3):
    ...
    home_last_against = get_n_last_matches_against_each_other(matches,
                                                              match_date,
                                                              home_team,
                                                              away_team, n=n2)
    away_last_against = get_n_last_matches_against_each_other(matches,
                                                              match_date,
                                                              away_team,
                                                              home_team, n=n2)
    # Goals stuff
...
def create_features(matches):
    ...
    matches_features = matches.apply(
        lambda x: get_features_for_match(x, matches, n1=12, n2=3), axis=1)
